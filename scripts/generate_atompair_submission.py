#!/usr/bin/env python3
"""
Generate submission_atompair.parquet using Atom Pair FP (2048-bit) + Morgan FP (512-bit) + 8 descriptors.
...
AP_BITS    = 2048
MORGAN_BITS = 512
TOTAL_BITS  = AP_BITS + MORGAN_BITS + len(DESC_COLS)  # 2568
...
def main() -> None:
    ...
    ap_gen     = rdFingerprintGenerator.GetAtomPairGenerator(fpSize=AP_BITS)
    ...
    print(
        f"Computing AtomPair({AP_BITS}bit) + Morgan({MORGAN_BITS}bit) + {len(DESC_COLS)} descriptors "
    ...
    for uid in scope_ids:
        ...
        if mol:
            ap_fp     = qnu.bitvect_to_array(ap_gen.GetFingerprint(mol), AP_BITS)
            morgan_fp = qnu.bitvect_to_array(morgan_gen.GetFingerprint(mol), MORGAN_BITS)
        else:
            ap_fp     = np.zeros(AP_BITS,     dtype=np.float32)
        ...
        x_train_rows.append(np.concatenate([ap_fp, morgan_fp, desc]))
    ...
    for i, (_, row) in enumerate(query.iterrows()):
        ...
        if mol is None:
            ...
            x_test_rows.append(np.concatenate([ap_fp, morgan_fp, np.array(desc_vals, dtype=np.float32)]))
    ...
    out_path = ROOT / "submission_atompair.parquet"
    submission.to_parquet(out_path)
    print(f"\nWrote {out_path} ({out_path.stat().st_size / 1e6:.2f} MB)", flush=True)
    print(
        f"Config: scope=qnu, ap_bits={AP_BITS}, morgan_bits={MORGAN_BITS}, "
