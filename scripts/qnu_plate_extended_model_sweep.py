...
def load_external_canon_maps() -> tuple[
    ...
    """Load external LPM embeddings. Returns empty dicts if paths are unavailable."""
    try:
        ...
        print(f"  WARNING: LPM embeddings not available ({e}). LPM-based features will be empty.")
        return {}, {}, {}, {}
...
def build_feature_sets(train_chem: pd.DataFrame, all_ids: list[str]) -> list[FeatureSet]:
    ...
    # Only add LPM-based feature sets if they have data
    if external_morgan:
        feature_sets.append(FeatureSet("external_morgan128", external_morgan))
        feature_sets.append(concat("external_morgan128+descriptors", external_morgan, desc_by_id))
    if lpm_all:
        feature_sets.append(FeatureSet("lpm_all_source", lpm_all))
        feature_sets.append(concat("lpm_all_source+descriptors", lpm_all, desc_by_id))
    if ft1:
        feature_sets.append(FeatureSet("lpm_ft_vcpi_0001", ft1))
    if ft2:
        feature_sets.append(FeatureSet("lpm_ft_vcpi_0002", ft2))
    if ft12:
        feature_sets.append(FeatureSet("lpm_ft_vcpi_0001_0002", ft12))
    if ft12_else_all:
        feature_sets.append(FeatureSet("lpm_ft12_else_all_source", ft12_else_all))
        feature_sets.append(concat("lpm_ft12_else_all_source+descriptors", ft12_else_all, desc_by_id))
